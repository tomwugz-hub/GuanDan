/**
 * 例1～5 场景 Top1 golden：批次A1（百例1～50补齐）
 */
...
function runStructure(n, spec) {
  ...
  assert(
    sPrefer < sOver,
    `例${n} structure 期望 ${spec.prefer.type}/${spec.prefer.mainRank} 优于 ${spec.over.type}/${spec.over.mainRank}，得 ${sPrefer} vs ${sOver}`,
  );
...
for (const n of [1, 2, 3, 4, 5]) {
  ...
  if (spec.kind === "open" && spec.over?.type !== "Pass") {
    runStructure(n, spec);
    runFollowOpen(n, spec);
  } else {
    runFollowOpen(n, spec);
  }
}

console.log("PASS 例1～5 场景 Top1 golden");
