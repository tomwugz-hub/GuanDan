...
function loadScenarios(n, kind = null) {
  ...
  for (const file of [
    ...
    for (const spec of JSON.parse(readFileSync(file, "utf8"))) {
      if (spec.caseNumber !== n) continue;
      if (kind && spec.kind !== kind) continue;
      out.push(spec);
    }
  }
  return out;
...
function runStructureCase(n, doctrine = "C100-G1") {
  const spec = loadScenarios(n, "structure").find((s) => s.doctrine === doctrine) ?? loadScenarios(n, "structure")[0];
...
runStructureCase(48, "C100-B1");
runCaseSpec(loadScenarios(49, "follow")[0], 49);
runCaseSpec(loadScenarios(50, "follow")[0], 50);
runStructureCase(51, "C100-G1");
runCaseSpec(loadScenarios(51, "follow")[0], 51);
runCaseSpec(loadScenarios(52, "follow")[0], 52);
console.log("PASS 例48～52 场景 Top1 golden");
