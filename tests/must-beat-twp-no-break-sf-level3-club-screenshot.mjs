/**
 * 级牌3 截图复现（match 须压 QQQ+55）：
 * 梅花 A2345 同花顺 + 红桃3逢人配，不宜拆跑道组 AAA+对J 压牌。
 */
...
const breakingAaaJj = classifyPlay([
  c("3", SUITS.hearts), c("A", SUITS.clubs), c("A", SUITS.spades),
  c("J", SUITS.diamonds), c("J", SUITS.spades),
], "3");

if (!breaksStrategicPremiumForTripleWithPair(breakingAaaJj, hand, "3", preferredGroups)) {
  throw new Error("截图 AAA+对J 应判定为拆同花顺/跑道");
}

function assertNotBreakingSfTop1(top, label) {
  if (top.candidate?.type !== PLAY_TYPES.tripleWithPair) return;
  if (top.candidate.mainRank !== "A") return;
  if (!breaksStrategicPremiumForTripleWithPair(top.candidate, hand, "3", preferredGroups)) return;
  throw new Error(`${label} 不应首推拆跑道的 AAA 三带二，实际 ${playSignature(top.candidate)}`);
}
...
console.log("PASS: 级牌3 梅花同花顺，须压 QQQ+55 不拆跑道首推 AAA+对J");
